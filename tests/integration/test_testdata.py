...
Testdata endpoints are not part of the simplified API, so we access
...
async def test_create_and_remove_subject(client: AsyncKSeF):
...
async def test_create_and_remove_person(client: AsyncKSeF):
    await client._ensure_auth()
    access_token = await client._get_access_token()
    test_nip = generate_random_nip()
